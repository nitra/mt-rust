...
**Дата:** 2026-07-27
**Статус:** ❌ **скасовано 2026-08-09 — реалізацію видалено, споживач не з'явився**
...
> **Чому скасовано.** Уся специфікація трималась на одному споживачі — `auto-worktree.mjs` у `@7n/rules`, який смикав `mt` через `spawnSync`. Через три дні, 2026-07-30, `@7n/rules` пішов іншим шляхом: власний Rust-стек `crates/rules-core` + `crates/rules-napi` із власним `worktree.rs`. `@7n/mt-napi` лишився без жодного споживача — ні крейта в цьому репозиторії, ні пакета в сусідніх, — тягнучи CI-матрицю zigbuild і два платформні npm-підпакети на кожен реліз. 2026-08-09 `crates/mt-napi`, `npm/mt-napi`, `packages/` і `npm-publish.yml` видалено.
>
> Опубліковані версії в npm (`@7n/mt-napi` 0.1.4 і платформні підпакети) **не знімались**: unpublish зламав би невідомих зовнішніх споживачів — публікація просто припинена. Якщо Node/Bun-споживач з'явиться, ця специфікація лишається готовим кресленням, а код — в історії git.
...
- Версія `.node`-аддону має бути прибита до версії `mt-core`, щоб уникнути дрейфу поведінки між CLI і napi-шляхом (та сама версійна схема, що й CLI-реліз `mt-v*`) — **уточнено під час реалізації, див. нижче**: це стосується лише головного `@7n/mt-napi`, не platform-пакетів.
...
**Доповнення після живого тестування релізу (рішення Д):** імена `@7n/mt-darwin-arm64`/`@7n/mt-linux-x64` виявились **не вільними** — вони вже опубліковані (версія `0.28.0`, 41 версія історії) зі старого репозиторію `nitra/mt`, де бандлили `mt-scanner`-бінарник + napi-аддон разом. npm відхиляє публікацію версії, нижчої за вже опубліковану, тож lockstep із workspace-версією `mt-core` (`0.1.x`) для цих двох пакетів неможливий. Рішення: platform-пакети далі несуть **лише** napi-аддон (інший корисний вантаж, ніж у старій версії) і версіонуються **окремо** від `mt-core` — власний monotonic-лічильник поверх останньої опублікованої версії, перший napi-only реліз — мінорний bump `0.29.0` (позначає змістовий розрив із форматом `0.28.0`), далі `patch++`. Головний пакет `@7n/mt-napi` (нове ім'я, без конфлікту) лишається lockstep із workspace-версією `mt-core`, як і планувалось; його `optionalDependencies` вказують на актуальну версію platform-пакетів, не на власну версію.
